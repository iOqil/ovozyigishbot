...
async def show_survey_details(callback: CallbackQuery, bot):
    ...
    # Check subscriptions
    async with aiosqlite.connect(DB_NAME) as db:
        # Check for specific linked channels
        async with db.execute("""
            ...
            WHERE sc.survey_id = ?
        """, (survey_id,)) as cursor:
            linked_channels = await cursor.fetchall()
            
        # If specific channels are linked, use them. 
        # If NOT linked, we should NOT enforce any subscription (according to plan).
        # Previously it checked all channels.
        
        channels_to_check = linked_channels
        
        # If you want fallback to "All channels" if none linked, uncomment below:
        # if not channels_to_check:
        #    async with db.execute("SELECT channel_id, name, url FROM channels") as cursor:
        #        channels_to_check = await cursor.fetchall()
        
    not_subscribed = []
    if channels_to_check:
        for ch_id, ch_name, ch_url in channels_to_check:
            try:

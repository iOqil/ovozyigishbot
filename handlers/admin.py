...
async def cmd_survey_channels(message: Message):
    ...
    text = "🔗 **Qaysi so'rovnoma uchun kanallarni sozlamoqchisiz?**"
    keyboard = []
    for s_id, title in surveys:
        keyboard.append([InlineKeyboardButton(text=f"⚙️ {title}", callback_data=f"sc_list_{s_id}")])
    
    markup = InlineKeyboardMarkup(inline_keyboard=keyboard)
    await message.answer(text, reply_markup=markup)

@router.callback_query(F.data.startswith("sc_list_"))
async def show_survey_channels(callback: CallbackQuery):
    survey_id = int(callback.data.split("_")[2])
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute("SELECT id, name FROM channels") as cursor:
            all_channels = await cursor.fetchall()
            
        # Get linked channels for this survey
        async with db.execute("SELECT channel_id FROM survey_channels WHERE survey_id = ?", (survey_id,)) as cursor:
    ...
    text = f"⚙️ **{title}** so'rovnomasi uchun majburiy kanallar:\n\nKanallar ustiga bosib ularni yoqing yoki o'chiring."
    
    keyboard = []
    for c_id, name in all_channels:
        is_linked = c_id in linked_ids
        status_icon = "✅" if is_linked else "❌"
        # Callback: sc_toggle_{survey_id}_{channel_id}
        keyboard.append([InlineKeyboardButton(
            text=f"{status_icon} {name}", 
            callback_data=f"sc_toggle_{survey_id}_{c_id}"
        )])
...
async def back_to_surveys_list(callback: CallbackQuery):
    # Reuse the logic from cmd_survey_channels but edit message
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT id, title FROM surveys WHERE is_active = 1") as cursor:
            surveys = await cursor.fetchall()
...
async def toggle_survey_channel(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        # Check if exists
        async with db.execute("SELECT 1 FROM survey_channels WHERE survey_id = ? AND channel_id = ?", (survey_id, channel_id)) as cursor:
            exists = await cursor.fetchone()
            
        if exists:
            # Delete
            await db.execute("DELETE FROM survey_channels WHERE survey_id = ? AND channel_id = ?", (survey_id, channel_id))
            action = "o'chirildi"
        else:
            # Add
            await db.execute("INSERT INTO survey_channels (survey_id, channel_id) VALUES (?, ?)", (survey_id, channel_id))
            action = "qo'shildi"
            
        await db.commit()
        
    await callback.answer(f"Kanal {action}!", show_alert=False)
    
    # Refresh list
    await show_survey_channels(callback)
